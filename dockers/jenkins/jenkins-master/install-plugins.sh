...
download() {
    ...
    echo "[download] $plugin, ${originalPlugin-None}, ${version-None}, ${lock-None}, ${ignoreLockFile-None}"
...
doDownload() {
	...
	local status=$?
	if [ $status -eq 0 ]; then
	  echo "Successfully created file ${plugin}"
	else
		if [ "$plugin" != "${plugin%-plugin}" ]; then
			echo "fallback plugin suffix"
			plugin=${plugin%-plugin}
			url="$JENKINS_UC_DOWNLOAD/plugins/$plugin/$version/${plugin}.hpi"
			curl --connect-timeout ${CURL_CONNECTION_TIMEOUT:-20} --retry ${CURL_RETRY:-5} --retry-delay ${CURL_RETRY_DELAY:-0} --retry-max-time ${CURL_RETRY_MAX_TIME:-60} -s -f -L "$url" -o "$jpi"
			status=$?
...
resolveDependencies() {
    local plugin jpi dependencies dep_jpi org_plugin
    plugin="$1"
    org_plugin="$1"
    jpi="$(getArchiveFilename "$plugin")"
    ...
    do
        ...
        elif [[ -e "$(getArchiveFilename "$plugin")" ]]; then
            echo "Skipping downloaded dependency $plugin"
        else
    ...
    echo "waiting for $org_plugin"
    wait
    echo "resolved dependency for [ $org_plugin ] => [ $dependencies ]"
}
